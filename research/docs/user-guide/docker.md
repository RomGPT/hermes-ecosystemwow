...
Set to `1` (or `true` / `yes`) to launch the dashboard alongside the main command

_(unset — dashboard not started)_
...
Set to `1` to expose the in-browser Chat tab (embedded `hermes --tui` via PTY/WebSocket)
...
2+ GB (grows with sessions/skills)
