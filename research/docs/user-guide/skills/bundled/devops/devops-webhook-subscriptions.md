...
-   Payload URL: the returned webhook\_url
...
Requires `--deliver` to be a real target (telegram, discord, slack, github\_comment, etc.) — `--deliver log` is rejected because log-only direct delivery is pointless.
